...
def employer(employer_id):
    ...
    jobs = execute_sql('SELECT job.id, job.title, job.description, '
                       'job.salary FROM job JOIN employer ON employer.id = job.employer_id WHERE '
...
def review(employer_id):
    if request.method == 'POST':
        ...
        execute_sql('INSERT INTO review (review, rating, title, date, '
                    'status, employer_id) VALUES (?, ?, ?, ?, ?, ?)', (review, rating,
                    title, date, status, employer_id), commit=True)
